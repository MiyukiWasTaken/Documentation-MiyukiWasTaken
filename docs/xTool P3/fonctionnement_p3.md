...
Pour mettre en marche la P3, activez l’interrupteur situé à l’arrière de la machine. La machine peut se déplacer brièvement pendant la phase de démarrage pour se calibrer, sans inquiétude. Profitez de ce calibrage pour préparer votre matériau ou charger votre fichier dans le logiciel xTool.
...
>NOTE : si la P3 n’est pas sélectionnée, cliquez sur les flèches <img src="../images/xTool P3/icone-changement-machine.png" alt="icône" width="18"> et choisissez la P3.
...
_**Attendez que la P3 arrête de ventiler avant de passer à l’étape suivante.**_

1. Ouvrez le couvercle vitré de la machine.
2. Vérifiez que les rails et barres de la zone de travail, surlignés en vert sur la première photo, sont **correctement positionnés** comme sur la deuxième photo.

<img src="../images/xTool P3/zone-de-travail.jpeg" alt="Zone de travail" width="600" style="margin-top:15px;">

<img src="../images/xTool P3/barres-zone-de-travail.png" alt="Zone de travail sans souci" width="600" style="margin-top:15px;">

Si tout est OK, posez votre matériau sur la zone de travail (zone de travail en vert, matériau en bleu).

**_Il est préférable de poser le matériau vers le milieu de la zone de travail plutôt qu’aux bords._**
...
Vous verrez ensuite, dans le logiciel xTool, la zone de travail telle qu’elle est réellement, avec le matériau posé dessus.
...
>NOTE : si la résolution du scan est mauvaise sur certaines parties, ou si vous souhaitez reprendre une photo d’un endroit précis après le scan général, refermez le couvercle et appuyez sur « Close shot ». Sélectionnez ensuite la zone voulue ; la tête prendra alors une photo ciblée.
...
Le laser situé dans la tête d’impression passera deux fois sur ce point : une fois avec la zone abaissée, puis avec la zone relevée.
...
Recherchez le matériau par nom ou épaisseur dans le champ de recherche encadré en violet, et/ou utilisez les catégories à gauche encadrées en bleu pour filtrer les résultats. Le plus simple est d’utiliser à la fois la recherche pour filtrer les matériaux à l’épaisseur souhaitée, puis les catégories pour sélectionner le bon matériau.
...
Avant d’exécuter le job, vérifiez manuellement que chaque process utilise la bonne puissance et la bonne vitesse.

Appuyez ensuite sur le bouton « Preview » pour obtenir une simulation numérique du résultat final. Cette prévisualisation permet de détecter rapidement les erreurs dans la planification des process.
...
Pendant le job, le logiciel affiche la vue d’une caméra à l’intérieur de la P3 ainsi que le temps restant estimé.
...
RESTEZ BIEN DEVANT LA P3 PENDANT L’**INTÉGRALITÉ** DU JOB.

Il est obligatoire de vérifier le bon déroulement du job pour éviter un départ d’incendie lorsque vous avez le dos tourné. Si vous devez, pour X ou Y raisons, vous éloigner de la machine, vous êtes dans l’obligation de mettre en pause la machine avec le même bouton qui permet de démarrer le job.

<img src="../images/xTool P3/job-en-cours.gif" alt="Animation du job en cours sur la P3" width="600" style="margin-top:15px;">

## Après le job

Une fois le job terminé, veuillez attendre **20 à 30 secondes** avant d’ouvrir le couvercle afin de vous assurer que *toutes les fumées soient extraites* par la ventilation.

Vous pouvez ensuite récupérer votre ou vos pièces découpées et/ou gravées en toute sécurité.
